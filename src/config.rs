...
pub fn get_grache_config(headers: &mut HeaderMap, query_params: HashMap<String, String>) -> GracheConfig {
    // parse ignore cookies
    let mut ignore_cookies = query_params.get("ignoreCookies")
        .and_then(|ic| ic.parse::<bool>().ok())
        .unwrap_or(false);

    ignore_cookies = headers.get("Grache_Ignore_Cookies")
    ...
    // parse expiration
    let mut expiration = query_params.get("ignoreCookies")
        .and_then(|ic| ic.parse::<i32>().ok())
        .unwrap_or(600);

    expiration = headers.get("Grache_Expiration")
